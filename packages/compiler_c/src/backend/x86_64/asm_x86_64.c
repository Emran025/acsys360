 ...
 for(size_t i=0;i<tac->count&&ok;i++){CTacInstruction *x=&tac->items[i];int dst=slot(semantic,x->result);
  switch(x->opcode){
   ...
   case C_TAC_READ: {
    ...
    int target=slot(semantic,x->result);
    int input_slot=(int)(frame+32+read_index*256);
    if(rt&&!strcmp(rt,"منطقي")){
     ok=target&&append(&result->text,&len,&cap,
       "read_bool_retry_%zu:\n"
       ...
       "    lea " ASM_FMT_REG ", [rel fmt_read_bool]\n"
       "    xor eax, eax\n"
       "    call scanf\n"
       ...
       "    mov qword [rbp-%d], 1\n"
       "read_bool_done_%zu:\n",
       read_index,read_index,input_slot,input_slot,read_index,input_slot,
       read_index,target,read_index,read_index,target,read_index);
    }else{
     const char *fmt=rt&&!strcmp(rt,"حقيقي")?"fmt_read_real":
       rt&&!strcmp(rt,"خيط_رمزي")?"fmt_read_str":
       rt&&!strcmp(rt,"حرفي")?"fmt_read_char":"fmt_read_int";
     int text_input=rt&&(!strcmp(rt,"خيط_رمزي")||!strcmp(rt,"حرفي"));
     ok=target&&append(&result->text,&len,&cap,
       "    lea " ASM_FMT_REG ", [rel fmt_input_request%zu]\n"
       "    xor eax, eax\n"
       "    call printf\n"
       ...
       "    xor eax, eax\n"
       "    call scanf\n",
       read_index,text_input?input_slot:target,fmt);
