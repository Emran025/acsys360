...
static int quoted(const char *s){return s&&((s[0]=='"'&&s[strlen(s)-1]=='"')||(s[0]=='\''&&s[strlen(s)-1]=='\''));}
...
static int emit_real(char **text,size_t *len,size_t *cap,const CSemanticResult *sem,const char *v,const char *type){
  ...
  int off=slot(sem,v); return off&&append(text,len,cap,"    movsd xmm0, [rbp-%d]\n",off);
}
...
static int emit_string(char **text,size_t *len,size_t *cap,const char *v){const char *p=v;if(*p=='"'||*p=='\'')p++;if(!append(text,len,cap,"    db "))return 0;int first=1;while(*p&&*p!='"'&&*p!='\''){unsigned char c=(unsigned char)*p++;if(c=='\\'&&*p){char e=*p++;if(e=='n')c='\n';else if(e=='t')c='\t';else c=(unsigned char)e;}if(!first&&!append(text,len,cap,", "))return 0;if(!append(text,len,cap,"%u",c))return 0;first=0;}return append(text,len,cap,", 0\n");}
int c_generate_nasm_x86_64(const CTacResult *tac,const CSemanticResult *semantic,CAssemblyResult *result){
 ...
 size_t request_count=0; for(size_t i=0;i<tac->count;i++)if(tac->items[i].opcode==C_TAC_READ&&!append(&result->text,&len,&cap,"fmt_input_request%zu: db \"input\",0\n",request_count++))goto fail;
 ...
 for(size_t i=0;i<tac->count&&ok;i++){CTacInstruction *x=&tac->items[i];int dst=slot(semantic,x->result);const char *t=type_of(semantic,x->result,x->type);
  switch(x->opcode){
   ...
   case C_TAC_BINARY:
    if(t&&!strcmp(t,"حقيقي")){ok=emit_real(&result->text,&len,&cap,semantic,x->left,x->type)&&append(&result->text,&len,&cap,"    sub rsp, 8\n    movsd [rsp], xmm0\n")&&emit_real(&result->text,&len,&cap,semantic,x->right,x->type)&&append(&result->text,&len,&cap,"    movsd xmm1, [rsp]\n    add rsp, 8\n");const char *op=x->operator;if(ok){if(!strcmp(op,"+"))ok=append(&result->text,&len,&cap,"    addsd xmm1, xmm0\n");else if(!strcmp(op,"-"))ok=append(&result->text,&len,&cap,"    subsd xmm1, xmm0\n");else if(!strcmp(op,"*"))ok=append(&result->text,&len,&cap,"    mulsd xmm1, xmm0\n");else if(!strcmp(op,"/"))ok=append(&result->text,&len,&cap,"    divsd xmm1, xmm0\n");else ok=0;if(ok)ok=append(&result->text,&len,&cap,"    movsd [rbp-%d], xmm1\n",dst);} }
   ...
   case C_TAC_UNARY: ok=emit_value(&result->text,&len,&cap,semantic,&strings,x->left,x->type);if(ok&&!strcmp(x->operator,"-"))ok=append(&result->text,&len,&cap,"    neg rax\n");else if(ok&&!strcmp(x->operator,"!"))ok=append(&result->text,&len,&cap,"    cmp rax, 0\n    sete al\n    movzx rax, al\n");if(ok)ok=append(&result->text,&len,&cap,"    mov [rbp-%d], rax\n",dst);break;
   case C_TAC_ASSIGN: if(t&&!strcmp(t,"حقيقي"))ok=emit_real(&result->text,&len,&cap,semantic,x->left,x->type)&&append(&result->text,&len,&cap,"    movsd [rbp-%d], xmm0\n",dst);else ok=emit_value(&result->text,&len,&cap,semantic,&strings,x->left,x->type)&&append(&result->text,&len,&cap,"    mov [rbp-%d], rax\n",dst);break;
   case C_TAC_PRINT: {const char *pt=type_of(semantic,x->left,x->type);if(pt&&!strcmp(pt,"حقيقي"))ok=emit_real(&result->text,&len,&cap,semantic,x->left,pt)&&append(&result->text,&len,&cap,"    movq rsi, xmm0\n    lea rdi, [rel fmt_real]\n    mov eax, 1\n    call printf\n");else if(pt&&!strcmp(pt,"منطقي"))ok=emit_value(&result->text,&len,&cap,semantic,&strings,x->left,pt)&&append(&result->text,&len,&cap,"    cmp rax, 0\n    jne bool_true_%zu\n    lea rdi, [rel fmt_bool_false]\n    xor eax, eax\n    call printf\n    jmp bool_done_%zu\nbool_true_%zu:\n    lea rdi, [rel fmt_bool_true]\n    xor eax, eax\n    call printf\nbool_done_%zu:\n",i,i,i,i);else {const char *fmt=pt&&(!strcmp(pt,"خيط_رمزي")||!strcmp(pt,"حرفي"))?"fmt_str":"fmt_int";ok=emit_value(&result->text,&len,&cap,semantic,&strings,x->left,pt)&&append(&result->text,&len,&cap,"    mov rsi, rax\n    lea rdi, [rel %s]\n    xor eax, eax\n    call printf\n",fmt);}break;}
   case C_TAC_READ: {const char *rt=type_of(semantic,x->result,x->type);const char *fmt=rt&&!strcmp(rt,"حقيقي")?"fmt_read_real":rt&&!strcmp(rt,"منطقي")?"fmt_read_bool":rt&&(!strcmp(rt,"خيط_رمزي")||!strcmp(rt,"حرفي"))?"fmt_read_str":"fmt_read_int";int target=slot(semantic,x->result);int text_input=rt&&(!strcmp(rt,"خيط_رمزي")||!strcmp(rt,"حرفي"));int input_slot=(int)(frame+32+read_index*256);ok=target&&append(&result->text,&len,&cap,"    lea rdi, [rel fmt_input_request%zu]\n    xor eax, eax\n    call printf\n    lea rsi, [rbp-%d]\n    lea rdi, [rel %s]\n    xor eax, eax\n    call scanf\n",read_index,text_input?input_slot:target,fmt);if(ok&&text_input)ok=append(&result->text,&len,&cap,"    lea rax, [rbp-%d]\n    mov [rbp-%d], rax\n",input_slot,target);read_index++;break;}
