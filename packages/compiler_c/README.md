...
سياسة backend الحالية صريحة: الأسّ الحقيقي (`^`) يُنفّذ عبر `pow` من `libm`،
أما الأسّ الصحيح السالب في مسار Native فينتج القيمة الصحيحة `0` بدل الدخول في
حلقة غير منتهية؛ يمكن لمسار interpreter تقييمه كقيمة حقيقية. البرنامج ذو TAC
الفارغ ينتج stub صالحًا لـ`main` ويمكن تحويله إلى artifact. استدعاءات الإجراءات
